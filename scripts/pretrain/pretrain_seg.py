...
from utils.util import set_seed,rank0_print,find_all_linear_names
...
def train(attn_implementation=None):
    ...
    print('ori_tokenizer_vocab_nums: ',ori_tokenizer_vocab_nums, ' MM_tokenizer_vocab_nums: ',MM_tokenizer_vocab_nums) # 151663
    ...
    model.get_model().init_multimodal_modules(visual_branch=training_args.visual_branch,
                                              ...
                                              d_model=4096,vit_ckpt_path=model_args.vit_ckpt_path,
    ...
    ckpt = torch.load('results/pretrain/llama-visual-qformer/non_lora_trainables.bin',map_location='cpu')
    weight = ckpt.pop('model.embed_tokens.weight',None)
    model.load_state_dict(ckpt,strict=False)
    print(f'pop embed weight, shape: {weight.shape}')
    
    for name, param in model.named_parameters():
        if 'seg_module' in name:
