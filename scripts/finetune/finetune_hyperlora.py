...
def train(attn_implementation=None):
	...
	if model_args.llm_name == 'llama':
		...
		config.iou_loss_weight = 0.0  # IoU regression head removed; keep field for compatibility
	...
	print('ori_tokenizer_vocab_nums: ',ori_tokenizer_vocab_nums,' MM_tokenizer_vocab_nums: ',MM_tokenizer_vocab_nums) # 增加了21个special token
	...
	# if 'iou_head' not in save_modules:
	# 	save_modules.append('iou_head')
	...
	if training_args.lora_enable:
		state_dict = get_peft_state_maybe_zero_3(model.named_parameters(), training_args.lora_bias) # lora参数
		non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(model.named_parameters()) # 非lora参数
